...
public class JournalEntryRunningBalanceUpdateServiceImpl implements JournalEntryRunningBalanceUpdateService {
    ...
    private void updateOrganizationRunningBalance(Date entityDate) {
        ...
        List<Map<String, Object>> list = jdbcTemplate.queryForList("select je.organization_running_balance as runningBalance,je.account_id as accountId from acc_gl_journal_entry je "
                ...
                + sqlGenerator.limit(10000, 0), entityDate, entityDate);
        ...
        List<Map<String, Object>> officesRunningBalanceList = jdbcTemplate.queryForList("select je.office_running_balance as runningBalance,je.account_id as accountId,je.office_id as officeId "
    ...
    private void updateRunningBalance(Long officeId, Date entityDate) {
        ...
        List<Map<String, Object>> list = jdbcTemplate.queryForList("select je.office_running_balance as runningBalance,je.account_id as accountId from acc_gl_journal_entry je "
                ...
                + "group by je.id order by je.entry_date DESC "
                + sqlGenerator.limit(10000, 0), officeId, entityDate, officeId, entityDate);
        for (Map<String, Object> entries : list) {
        ...
        for (JournalEntryData entryData : entryDatas) {
            BigDecimal runningBalance = calculateRunningBalance(entryData, runningBalanceMap);
            String sql = "UPDATE acc_gl_journal_entry SET office_running_balance=" + runningBalance + " WHERE id="
                    + entryData.getId();
            updateSql[i++] = sql;
        }
