...
public class ChargeWritePlatformServiceJpaRepositoryImpl implements ChargeWritePlatformService {
    ...
    private boolean isAnyLoansAssociateWithThisCharge(final Long chargeId) {
        ...
        final String sql = "select if((exists (select 1 from m_loan_charge lc where lc.charge_id = ? and lc.is_active = true)) = 1, 'true', 'false')";
    ...
    private boolean isAnySavingsAssociateWithThisCharge(final Long chargeId) {
        ...
        final String sql = "select if((exists (select 1 from m_savings_account_charge sc where sc.charge_id = ? and sc.is_active = true)) = 1, 'true', 'false')";
